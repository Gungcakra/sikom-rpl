...
class Pengurus extends Component
{
    ...
    public $id_pengurus, $id_anggota, $jabatan, $periode_mulai, $periode_akhir, $idToDelete, $search = '', $showForm = false, $userRole;
    ...
    public function mount()
    {
        ...
        if (Auth::user()->roles->pluck('name')->first() === 'admin') {
            $this->userRole = 'admin';
        } else if (Auth::user()->roles->pluck('name')->contains('pengurus')) {
    ...
    public function render()
    {
        ...
        if ($this->userRole == 'admin') {
            ...
            $idAnggotaUser = Anggota::where('id_user', Auth::id())->pluck('id_anggota');

            // Ambil id_organisasi dari anggota milik user ini
            $idOrganisasi = Anggota::whereIn('id_anggota', $idAnggotaUser)->pluck('id_organisasi')->first();

            // Ambil semua id_anggota yang tergabung dalam organisasi yang sama
            $anggotaOrganisasi = Anggota::where('id_organisasi', $idOrganisasi)->pluck('id_anggota');
            ...
            return view('livewire.pages.admin.masterdata.pengurus.index', [
                ...
                'anggotas' => Anggota::where('id_organisasi', $idOrganisasi)
                    ->whereNotIn('nim', $nimsSudahPengurus)
                    ->get(),
            ]);
        }
    }
